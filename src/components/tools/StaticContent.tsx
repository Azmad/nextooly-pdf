...
const PDF_TOOL_LINKS = [
  {
    name: "Compress PDF",
    desc: "Reduce PDF size while keeping text sharp and selectable.",
    href: "/compress",
  },
  {
    name: "Edit PDF",
    desc: "Add text, signatures, shapes, redactions, and quick text fixes in your browser.",
    href: "/pdf-edit",
  },
  {
    name: "Protect PDF",
    desc: "Encrypt PDFs with a password locally in your browser.",
    href: "/protect-pdf",
  },
  {
    name: "Unlock PDF",
    desc: "Remove a known PDF password privately without uploading the file.",
    href: "/unlock-pdf",
  },
];

export function MoreToolsSection({ currentPath }: MoreToolsSectionProps) {
  const tools = PDF_TOOL_LINKS.filter((tool) => tool.href !== currentPath);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {tools.map((tool) => (
          <Link
            key={tool.href}
            href={tool.href}
            className="group flex flex-col p-3 rounded-xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-all min-h-[140px]"
      ...
      <p className="mt-4 text-sm text-slate-500">
        Looking for tools outside the PDF suite?{" "}
        <a
          href="https://nextooly.com/"
          className="text-blue-600 hover:underline"
        >
          Explore the full Nextooly catalog
        </a>
        .
      </p>
    </section>
